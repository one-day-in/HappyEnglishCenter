...
export default function HomeworkDetailClient({ id }: { id: string }) {
  ...
  const groupStudents = mockStudents.slice(0, 4)
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-2xl font-bold flex-1 truncate" style={{ letterSpacing: '-0.01em' }}>
          {hw.title}
        </h2>
      </div>
      ...
      <Card className="border border-border/60 shadow-sm rounded-2xl bg-white">
        <CardContent className="pt-6 space-y-4">
          ...
          {hw.description && (
            <div className="rounded-xl p-4" style={{ background: 'rgba(107,58,58,0.05)' }}>
      ...
      <Card className="border border-border/60 shadow-sm rounded-2xl bg-white">
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center justify-between" style={{ letterSpacing: '-0.01em' }}>
            ...
            return (
              <div
                ...
                className={`flex items-center justify-between p-3 rounded-xl transition-colors ${
                  done ? 'bg-emerald-50 border border-emerald-100' : 'bg-muted/50 hover:bg-muted'
                }`}
              >
                <div className="flex items-center gap-3">
                  <div
                ...
                <div className="flex items-center gap-2">
                  {done ? (
                    <Badge className="bg-emerald-100 text-emerald-700 border-emerald-200 flex items-center gap-1 text-xs">
